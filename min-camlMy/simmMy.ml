...
and g' env = function (* 各命令の13bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | SLL(x, V(y)) -> assert false;	(* 即値最適化できないことはありえない? *)
  | SRA(x, V(y)) when M.mem y env -> SRA(x, C(M.find y env))
  | SRA(x, V(y)) -> assert false;	(* 同上 *)
  ...
  (* | IfEq(x, V(y), e1, e2) when M.mem y env -> IfEq(x, C(M.find y env), g env e1, g env e2) *)(* mipsに即値比較ブランチがない *)
