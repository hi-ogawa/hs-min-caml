...
let rec target' src (dest, t) = function	(* 関数呼び出しがあったらtrueを返す *)
...
and target src dest = function (* register targeting (caml2html: regalloc_target) *)
  ...
  | Let(xt, exp, e) ->			(*expが関数呼び出しの時はeを見ない*)
...
let rec alloc dest cont regenv x t =	(* Letの目的変数であるxをregに割り当てに行く *)
  (* allocate a register or spill a variable *)
  (* Printf.printf "debug_print(alloc): %s\n" x; *)
  ...
  let free = fv cont in
  try
    let (c, prefer) = target x dest cont in	(* c(bool値)は使わない *)
    let live = (* 生きているレジスタ *)
      List.fold_left
        (fun live y ->
  ...
  with Not_found ->
    ...
    let y = (* 型の合うレジスタ変数を探す *)
	  ...
	  not (is_reg y) &&
        ...
        (List.rev free) in	(* 最後の方に使われるやつから見てく(reverse) *)
...
(* (目的reg, Type) -> (後続の命令列asm.t) -> (reg割当map) -> (今の命令列asm.t)  ==>>  (regenv, asm.t) *)
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  | Ans(exp) -> g'_and_restore dest cont regenv exp
  | Let((x, t) as xt, exp, e) ->
      assert (not (M.mem x regenv));	(* xはまだ割り当てられてないはず *)
